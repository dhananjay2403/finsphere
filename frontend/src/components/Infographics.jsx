...
export function FlowDiagram({ steps, title }) {
  return (
    <DiagramWrapper title={title}>
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, alignItems: { xs: 'stretch', sm: 'flex-start' }, gap: { xs: 0, sm: 0 } }}>
        {steps.map((step, i) => (
          <Box key={i} sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, alignItems: 'center', flex: 1 }}>
            <Box
              sx={{
                flex: 1,
                p: { xs: 1.5, sm: 2 },
                bgcolor: i === 0 ? COLORS.softPink : COLORS.beige,
                border: '1px solid',
            ...
            >
              <Box
                sx={{
                  ...
                  bgcolor: PALETTE[i % PALETTE.length], color: '#fff',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontWeight: 700, fontSize: '0.7rem', mx: 'auto', mb: 1,
                }}
              >
                {i + 1}
              </Box>
              ...
              {step.detail && (
                <Typography variant="caption" color="text.secondary" sx={{ mt: 0.5, display: 'block', fontSize: '0.68rem', lineHeight: 1.4 }}>
                  {step.detail}
                </Typography>
              )}
            </Box>
            {i < steps.length - 1 && (
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', py: { xs: 0.5, sm: 0 }, px: { sm: 0.5 } }}>
                <svg width="20" height="20" viewBox="0 0 20 20" style={{ display: 'block', transform: 'rotate(90deg)' }}>
                  <path d="M10 3 L10 15 M6 11 L10 15 L14 11" stroke={COLORS.burgundy} strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </Box>
            )}
          </Box>
...
export function ComparisonDiagram({ left, right, title }) {
  return (
    <DiagramWrapper title={title}>
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 2 }}>
        <Box sx={{ p: 2, bgcolor: COLORS.softPink, borderRadius: 2, border: '1px solid', borderColor: 'primary.main' }}>
          <Typography variant="body2" fontWeight={700} color="primary.main" mb={1.5} sx={{ fontSize: '0.82rem' }}>
            {left.title}
          </Typography>
          {left.items.map((item, i) => (
            <Box key={i} sx={{ display: 'flex', alignItems: 'flex-start', gap: 1, mb: 0.75 }}>
              <Box sx={{ width: 6, height: 6, borderRadius: '50%', bgcolor: COLORS.burgundy, flexShrink: 0, mt: 0.7 }} />
              <Typography variant="caption" color="text.secondary" sx={{ fontSize: '0.72rem', lineHeight: 1.5 }}>{item}</Typography>
            </Box>
          ))}
        </Box>
        <Box sx={{ p: 2, bgcolor: COLORS.beige, borderRadius: 2, border: '1px solid', borderColor: 'divider' }}>
          <Typography variant="body2" fontWeight={700} color="text.primary" mb={1.5} sx={{ fontSize: '0.82rem' }}>
            {right.title}
          </Typography>
          {right.items.map((item, i) => (
            <Box key={i} sx={{ display: 'flex', alignItems: 'flex-start', gap: 1, mb: 0.75 }}>
              <Box sx={{ width: 6, height: 6, borderRadius: '50%', bgcolor: COLORS.clay, flexShrink: 0, mt: 0.7 }} />
              <Typography variant="caption" color="text.secondary" sx={{ fontSize: '0.72rem', lineHeight: 1.5 }}>{item}</Typography>
            </Box>
          ))}
        </Box>
...
export function AllocationDiagram({ segments, title }) {
  return (
    <DiagramWrapper title={title}>
      <Box sx={{ display: 'flex', height: 28, borderRadius: 2, overflow: 'hidden', mb: 2 }}>
        {segments.map(({ label, pct, color }, i) => (
          <Box
            ...
            sx={{
              width: `${pct}%`,
              bgcolor: color || PALETTE[i % PALETTE.length],
              display: 'flex', alignItems: 'center', justifyContent: 'center',
      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: { xs: 1, sm: 2 } }}>
        {segments.map(({ label, pct, color }, i) => (
          <Box key={label} sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
            <Box sx={{ width: 10, height: 10, borderRadius: '50%', bgcolor: color || PALETTE[i % PALETTE.length], flexShrink: 0 }} />
...
export function TimelineDiagram({ events, title }) {
  return (
    <DiagramWrapper title={title}>
      <Box sx={{ position: 'relative', pl: { xs: 3, sm: 4 } }}>
        <Box
          sx={{
            position: 'absolute', left: { xs: 10, sm: 14 },
            top: 8, bottom: 8, width: 2,
            bgcolor: COLORS.divider, borderRadius: 1,
          }}
        />
        {events.map((event, i) => (
          <Box key={i} sx={{ position: 'relative', pb: i < events.length - 1 ? 2.5 : 0 }}>
            <Box
              sx={{
                position: 'absolute', left: { xs: -22, sm: -26 }, top: 4,
                width: 14, height: 14, borderRadius: '50%',
                bgcolor: PALETTE[i % PALETTE.length],
                border: '2px solid', borderColor: COLORS.white,
                boxShadow: `0 0 0 2px ${PALETTE[i % PALETTE.length]}33`,
              }}
            />
            ...
            {event.detail && (
              <Typography variant="caption" color="text.secondary" sx={{ fontSize: '0.7rem', lineHeight: 1.5, mt: 0.25, display: 'block' }}>
                {event.detail}
              </Typography>
            )}
          </Box>
...
export function ProcessDiagram({ steps, title, centerLabel }) {
    ...
    <DiagramWrapper title={title}>
      <Box sx={{ display: 'flex', justifyContent: 'center' }}>
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ maxWidth: '100%' }}>
          {centerLabel && (
            <text x={cx} y={cy} textAnchor="middle" dominantBaseline="central" fontSize="11" fontWeight="700" fill={COLORS.burgundy}>
              {centerLabel}
            </text>
          )}
          {steps.map((step, i) => {
            ...
            return (
              <g key={i}>
                <line x1={x} y1={y} x2={nx} y2={ny} stroke={COLORS.divider} strokeWidth="1.5" strokeDasharray="4 3" />
                <circle cx={x} cy={y} r={18} fill={color} />
                <text x={x} y={y - 4} textAnchor="middle" dominantBaseline="central" fontSize="9" fontWeight="700" fill="#fff">
                  {i + 1}
                </text>
                <text x={x} y={y + 7} textAnchor="middle" dominantBaseline="central" fontSize="6" fontWeight="500" fill="rgba(255,255,255,0.85)">
                  {step.label.length > 10 ? step.label.slice(0, 10) + '…' : step.label}
                </text>
              </g>
            );
          })}
